...
`RESULT: OK` / `DEGRADED` / `FAILED` を明示する。
**ただし health を過信せず、coverage の週数は引き続き必ず目視すること。**

> ### ⚠️ 旧ルール「実行時間が短ければ失敗を疑う」は使えない（2026-08-09 訂正）
> 本書は従来「**実行時間が異常に短い（例: 全年で18秒）場合は取得失敗を疑う**」と
> していたが、**この判定基準は誤り**であることが実測で判明した。
>
> 2026-08-09 のフルバックフィル（2005-2026・全33URL）は
> **293.4 MB を 6.1秒で取得して完全に成功**した（run 31322502500）。
> GitHub Actions ランナーの回線が非常に速いため、成功時のほうが18秒より短い。
> 旧ルールのままだと**成功を失敗と誤判定する**。
>
> 正しい判定材料は health ブロックの以下2つ:
> - `fetch failures : 0` であること
> - `downloaded : 293.4 MB` のように**ダウンロード量がMB単位で出ている**こと
>   （全失敗時は `0.0 MB` になる。実測で確認済み）
>
> 実行時間は成否の判断に使わない。
...
**公開URLとレポート側の参照はそのまま使える**（SPEC §10-3 / §10-7）。

さらに、CFTC公式ZIPからの**フルバックフィル（2005-2026・33URL・293.4MB）で
19本のCSVすべてが byte-identical に再現された**（SPEC §10-5）。
保存済みデータが現在の公式公表値と完全に一致していることの実証にあたる。
