...
def main():
    ...
    # 「古い」と「そもそも無い」は原因も対処も違うので分けて報告する。
    # 一緒くたにすると、CSV未生成の銘柄が max_stale_days を無視して
    # 常に赤くなり、しかも理由が「鮮度」と表示されて誤解を招く。
    stale_syms, nodata_syms = [], []
    for s in SYMBOLS:
        limit = s.get("max_stale_days", STALE_AFTER_DAYS)
        ds = sorted(read_symbol_csv(s["slug"]))
        if not ds:
            nodata_syms.append("%s(legacy)" % s["slug"])
        elif _stale(ds[-1]) > limit:
            stale_syms.append("%s(legacy:%s/許容%d日)" % (s["slug"], ds[-1], limit))
    tff_dates = []
    for s in tff_syms:
        limit = s.get("max_stale_days", STALE_AFTER_DAYS)
        ...
        if not ds:
            nodata_syms.append("%s(tff)" % s["slug"])
        elif _stale(ds[-1]) > limit:
            stale_syms.append("%s(tff:%s/許容%d日)" % (s["slug"], ds[-1], limit))
    ...
    if nodata_syms:
        problems.append("CSVが存在しない系列: %s（backfill-history の実行が必要）"
                        % ", ".join(nodata_syms))
