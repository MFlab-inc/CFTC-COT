...
- **状態表示（v1.2）**：偏り度（全履歴パーセンタイル）・勢い（4週差分）・AM×LevF整合状態を
  JSONの`state`ブロックとダッシュボードのバッジで表示。仮置き閾値による参考情報で売買助言ではない
...
このリポジトリで過去に起きた事故はいずれも「取得に失敗しているのにワークフローが
緑✅で終わる」形だったため、機械判定を入れてあります（詳細は
[docs/SPEC.md](docs/SPEC.md) §11）。

- 両ワークフローともログ末尾に `=== health ===` を出力し `RESULT: OK / DEGRADED / FAILED` を明示
- `backfill-history` は取得失敗・マッチ0件・**週数の減少**があると異常終了し、
  commitステップごと飛ばす（＝中途半端なデータはリポジトリに入らない）
- `weekly-cot-update` はLegacyを必ずcommitしたうえで、TFF失敗や鮮度異常があれば
  runを赤くする（取れたデータを捨てないため、この順序が重要）
- **health が OK でも coverage の週数は目視すること**

`backfill-history` の `start_year` は「どの年のZIPを取りに行くか」だけを決めます。
範囲外の既存データが削られることはありません。
